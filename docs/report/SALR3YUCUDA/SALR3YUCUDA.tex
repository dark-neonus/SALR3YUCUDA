...
\begin{abstract}
We present a computational framework for solving mean-field density functional theory (DFT) equations describing spatial density distributions of binary colloidal mixtures with competing short-range attraction and long-range repulsion (SALR) interactions. The solver implements the Picard iterative method for a two-component system with different walls configuration, using the triple Yukawa potential model. Our CPU implementation employs OpenMP parallelization, achieving significant speedup on multi-core processors. A robust CUDA implementation for GPUs was developed, delivering up to $\approx 40\times$ faster runtime compared to the single-threaded CPU version and a $\sim 5\times$ speedup compared to maximum OpenMP parallelization. We demonstrate the solver capabilities by computing equilibrium density profiles under periodic, two-wall, and four-wall boundary conditions. A Qt-based graphical user interface (GUI) was also built to facilitate parameter configuration, task execution, and result visualization.
\end{abstract}
...
On a multi-core processor (Intel Core i7-13700H, 20 threads), this OpenMP parallelization achieves up to a $7\times$ speedup compared to single-threaded execution on the largest grid sizes ($150\times 150$).
...
The project integrates a comprehensive Qt 6-based Graphical User Interface developed for end-to-end simulation lifecycle management. Parameters such as temperature, grid shape, and boundary constraints are defined seamlessly. More essentially, the GUI wraps an underlying SQLite and HDF5 backend which ensures persistent tracking of thermodynamic constraints, density tensors over iterative runs, and allows live visual representation (heatmaps and scatter plots) directly from the persistent `.h5` artifacts, eliminating the overhead of manual data handling.

\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{src/gui_placeholder.png}}
\caption{Screenshot of the Qt Visualization GUI. The interface features parameter control, run initialization, database session browsing, and real-time visualization of density fields via heatmaps and scatter plots.}
...
A scalability analysis details the runtime throughput in terms of convolution evaluation per grid volume. Using a $150\times 150$ grid, the single-threaded OpenMP approach required $764$s execution time. Under maximal (20-thread) CPU allocation, time converged bounding closer to $100$s. 

In sharp contrast, the corresponding execution on an NVIDIA GeForce RTX 4060 Laptop GPU achieved complete iteration to convergence in just $19.2$s. This corresponds to an aggregated speedup over $39.7\times$ against the fundamental serial baseline and a $\sim 5\times$ speed gain explicitly over full CPU thread saturation. Figure ~\ref{fig:performance} demonstrates the drastic acceleration achieved via the CUDA framework against variable OpenMP implementations.

\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{../../analysis/results/13th_Gen_Intel_R_Core_TM_i7-13700H_NVIDIA_GeForce_RTX_4060_Laptop_GPU/speedup_analysis.png}}
\caption{Overall Speedup (vs. single-threaded CPU runtime) corresponding to variable problem geometries ($16 \times 16$ to $150 \times 150$) emphasizing the asymptotic supremacy of execution on CUDA architecture.}
...
Specifically, the deployment on CUDA effectively shrinks days-long iterative experiments on large grids to mere minutes, bringing rapid exploration and phase-space sweeping into realistic ranges. While CPU cores exhibit efficiency plateaus and bounded bandwidths as cache lines overextend during grid sweeps, the CUDA architecture inherently processes continuous convolutions effectively, maintaining almost steady-state high throughput unimpacted by sequential bounds.

%--------------------------
\section{Conclusion}
%--------------------------

We have presented SALR3YUCUDA, a computational solver for mean-field DFT equations describing binary SALR colloidal mixtures. The CPU implementation with OpenMP parallelization and, crucially, a highly optimized GPU accelerator in CUDA successfully compute equilibrium density distributions under various boundary conditions, demonstrating stripe and cluster formation characteristic of competing-interaction systems in fractions of previous iteration times.

Demonstrated benchmarks show the system effortlessly handling grids up to $150^2$, effectively outperforming parallel CPU cores by almost five-fold and single threads by $\approx 40\times$. 
Furthermore, the integrated `.h5` backed Qt GUI serves a robust pipeline for iterative testing and on-the-fly analytical visualization of distributions.
