...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.92\columnwidth]{src/pbc-random.png}}
\caption{PBC with random starting distribution. Combined 2D and 3D snapshot.}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.92\columnwidth]{src/pbc-sinusoidal.png}}
\caption{PBC with sinusoidal starting distribution. Combined 2D and 3D snapshot.}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.92\columnwidth]{src/rep_pbc.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.92\columnwidth]{src/w2-random.png}}
\caption{W2 with random starting distribution. Combined 2D and 3D snapshot.}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.92\columnwidth]{src/w2-sinusoidal.png}}
\caption{W2 with sinusoidal starting distribution. Combined 2D and 3D snapshot.}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.92\columnwidth]{src/rep_w2.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.92\columnwidth]{src/w4-random.png}}
\caption{W4 with random starting distribution. Combined 2D and 3D snapshot.}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.92\columnwidth]{src/w4-sinusoidal.png}}
\caption{W4 with sinusoidal starting distribution. Combined 2D and 3D snapshot.}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.92\columnwidth]{src/rep_w4.png}}
...
We focus on two primary metrics: \textit{execution time} and \textit{speedup}.

\textbf{Execution time} is measured as the total wall-clock time from the start of the iteration loop to convergence (or the maximum iteration limit). It is the most direct indicator of solver efficiency and directly determines how quickly a researcher can sweep across the parameter space. Disk I/O is excluded to isolate the compute-only cost.

\textbf{Speedup} is defined as:
\begin{equation}
S = \frac{t_{\text{ref}}}{t_{\text{target}}}
\end{equation}
where $t_{\text{ref}}$ is the reference execution time (typically single-thread CPU or a fixed baseline) and $t_{\text{target}}$ is the execution time of the configuration under evaluation. We report two kinds of speedup: CUDA over CPU (to quantify GPU benefit) and multi-thread CPU over single-thread CPU (to quantify OpenMP scaling efficiency). These two metrics together reveal both absolute performance gains and scaling behavior as compute resources increase.

\subsubsection{Benchmark parameters and hardware}

The performance benchmarks use a dedicated benchmark configuration with disk I/O disabled. The same physical parameters as the default solver run are used. Grid sizes and thread counts follow the measured dataset from the laptop benchmark logs:

\begin{itemize}
    \item Grid sizes: $N \in \{16, 32, 64, 128, 160, 256, 320\}$ (square grids, $N \times N$ nodes).
...
RAM & 16 GB DDR5-4800 (2$\times$8 GB SODIMM) \\
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{src/performance_comparison.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{src/performance_real_params_laptop.png}}
...
Cluster hardware details captured by the benchmark logs are summarized in Table~\ref{tab:cluster_hw}. The AMD EPYC 9754 is a server-grade processor in a dual-socket configuration (2$\times$128 cores, 256 cores total), designed for high-core-count parallel workloads. Its large core count allows the OpenMP convolution to distribute across many threads with minimal per-thread overhead. The AMD Threadripper PRO 5975WX is a workstation processor with 32 cores (64 threads), well suited for compute-intensive desktop workloads. The RTX 5090 is NVIDIA's flagship Blackwell-generation GPU with 32~GB of VRAM, enabling very large grid runs and significantly higher theoretical throughput than the RTX 4060 Laptop. Additional hardware properties such as cache sizes and memory bandwidth for the cluster nodes were not recorded by the benchmark logs.

\begin{table*}[t]
\caption{Cluster hardware used for benchmarks. Nodes were allocated exclusively to ensure clean timing results.}
\label{tab:cluster_hw}
\begin{center}
\begin{tabularx}{\textwidth}{|l|X|c|c|l|c|}
\hline
\textbf{Node (queue)} & \textbf{CPU model} & \textbf{CPU cores} & \textbf{Max threads used} & \textbf{GPU} & \textbf{VRAM} \\
\hline
node-003 (\texttt{job-parallel-node}) & AMD EPYC 9754 & 256 (2$\times$128) & 64 & None & N/A \\
\hline
gnode-003 (\texttt{job-parallel-gnode}) & AMD Threadripper PRO 5975WX & 32 & 16 & None & N/A \\
\hline
gnode-003 (\texttt{job-parallel-gpu}) & AMD Threadripper PRO 5975WX & 32 & 1 (host) & NVIDIA GeForce RTX 5090 & 32607 MiB \\
\hline
\end{tabularx}
\end{center}
\end{table*}
...
Figure~\ref{fig:cluster_cpu_log} compares the best CPU time per grid size across the laptop and cluster nodes on a log scale. The EPYC 9754 achieves the lowest CPU runtimes due to its high core count, with 64-thread scaling yielding the best absolute times. The Threadripper PRO 5975WX shows intermediate performance with up to 16 threads.

Figure~\ref{fig:cluster_thread_speedup} summarizes OpenMP thread-scaling speedup at the largest available grid size for each machine. The EPYC node scales to 64 threads; the Threadripper node reaches 16. Both plateau well below the linear ideal, consistent with the memory-bound nature of the $O(N^2)$ convolution.

Figures~\ref{fig:cluster_gpu_runtime} and~\ref{fig:cluster_speedup} compare GPU versus CPU runtimes and show speedup of each machine relative to the single-thread laptop baseline. The RTX 5090 on the cluster GPU node delivers the best GPU performance, reflecting both the larger VRAM and higher CUDA throughput compared to the laptop RTX 4060.

\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{src/cpu_runtime_log.png}}
\caption{Best CPU runtime across laptop and cluster machines (log scale).}
\label{fig:cluster_cpu_log}
\end{figure}

\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{src/thread_scaling_speedup.png}}
\caption{OpenMP speedup versus single-thread baseline for the largest available grid size.}
\label{fig:cluster_thread_speedup}
\end{figure}

\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{src/gpu_runtime.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{src/speedup_vs_laptop.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{src/performance_real_params_cluster.png}}
...
\bibitem{chacko2015two}Chacko, B., Chalmers, C. \& Archer, A. Two-dimensional colloidal fluids exhibiting pattern formation. {\em The Journal Of Chemical Physics}. \textbf{143} (2015)

\bibitem{pini2017pattern}Pini, D. \& Parola, A. Pattern formation and self-assembly driven by competing interactions. {\em Soft Matter}. \textbf{13}, 9259-9272 (2017)

\bibitem{kravtsiv2022soft}Kravtsiv, I., Patsahan, T., Holovko, M. \& Di Caprio, D. Soft core fluid with competing interactions at a hard wall. {\em Journal Of Molecular Liquids}. \textbf{362} pp. 119652 (2022)

\bibitem{cuda_guide}
NVIDIA Corporation, ``CUDA C++ Programming Guide,'' v12.4, 2024. [Online]. Available: \url{https://docs.nvidia.com/cuda/cuda-programming-guide/}. [Accessed: 7-May-2026].

\bibitem{roth2006introduction}Roth, R. Introduction to density functional theory of classical systems: Theory and applications. {\em Lecture Notes, November}. (2006)
