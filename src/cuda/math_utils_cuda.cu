/* math_utils_cuda.cu - CUDA vector ops and reductions */
...
static int ensure_workspace(int blocks) {
    if (!g_workspace_registered) {
        atexit(free_workspace);
        g_workspace_registered = 1;
    }
    ...
    int new_size = (blocks < MAX_BLOCKS) ? MAX_BLOCKS : blocks;
    double *new_h_workspace = (double *)malloc((size_t)new_size * sizeof(double));
    if (!new_h_workspace) {
        return 0;
    }

    double *new_d_workspace = NULL;
    if (cudaMalloc(&new_d_workspace, (size_t)new_size * sizeof(double)) != cudaSuccess) {
        free(new_h_workspace);
        return 0;
    }

    free_workspace();
    g_d_workspace = new_d_workspace;
    g_h_workspace = new_h_workspace;
    g_workspace_size = new_size;
    return 1;
}

/* Warp sum with shuffle. */
...
/* Block sum via per-warp partials. */
